...
//        $this->db->from("query_response","PARTITION($dyn_part)");		        
        ...
        $where = " ((enquiry.created_by=$user_id OR enquiry.aasign_to=$user_id OR ticket.assign_to=$user_id OR ticket.assigned_by=$user_id OR visit.user_id=$user_id) OR query_response.create_by=$user_id OR query_response.related_to=$user_id)  AND CONCAT(str_to_date(task_date,'%Y-%m-%d'),' ',task_time) <= NOW() ORDER BY CONCAT(str_to_date(task_date,'%Y-%m-%d'),' ',task_time) DESC";
        $this->db->where($where);
        $this->db->limit($limit);
...
//         $this->db->from("query_response","PARTITION($dyn_part)");		        
         ...
         $where = " ((enquiry.created_by=$user_id OR enquiry.aasign_to=$user_id OR visit.user_id=$user_id) OR query_response.create_by=$user_id OR query_response.related_to=$user_id)  AND query_response.noti_read=0 AND CONCAT(str_to_date(task_date,'%Y-%m-%d'),' ',task_time) <= NOW() ORDER BY CONCAT(str_to_date(task_date,'%Y-%m-%d'),' ',task_time) DESC";
         $this->db->where($where);
