       ...
       $res= array();
        ...
        $data['result'] = $this->enquiry_model->deal_list_api($company_id,$user_id,$process,$limit,$offset);
          ...
          if(!empty($data['result']->result()))
          {
            ...
            {
              array_push($res['list'],array('id'=>$value->id,'name'=>$value->name,'enquery_id'=>$value->enquiry_id,'booking_type'=>$value->booking_type,'business_type'=>$value->business_type.'ward','deal_type'=>$value->deal_type,'company'=>$value->company_name,'client_name'=>$value->client_name,'status'=>$value->status,'creation_date'=>$value->creation_date,'quotation_url'=>base_url('dashboard/pdf_gen/').$value->id,'edited'=>$value->edited));  
            } 
            $this->set_response([
                'status' => TRUE,
                'data' =>$res
