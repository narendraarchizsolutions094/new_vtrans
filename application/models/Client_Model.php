...
//print_r($id_array);exit;
        ...
        $this->db->select('contacts.cc_id,contacts.comp_id,contacts.client_id,contacts.c_name,contacts.contact_number,contacts.emailid,contacts.other_detail,contacts.created_at,contacts.decision_maker,tbl_designation.desi_name as designation,enquiry.company,enquiry.enquiry_id,concat_ws(" ",name_prefix,name,lastname) as enq_name,comp.company_name');
    ...
    public function getCompanyList($id =0,$where_arr=array(),$comp_id=0,$user_id=0,$process=0,$action='data',$limit=-1,$offset=-1,$sort=-1)
    {
        ...
        $where="comp.comp_id=".$comp_id;
         if($user_id!=-1)
        {
            $this->db->join('enquiry enq','enq.company=comp.id','left')->group_by('comp.id');
            if(!empty($user_id))
            {
	...
	public function getCompanyList_visit()
    {
...
//End
        ...
        $this->db->where($where);
        $this->db->where('comp.dumy_company!=','1');
        if(is_array($process)){
            $this->db->where_in('comp.process_id',$process);
        }else{ 
            $this->db->where(' comp.process_id IN ('.$process.') ');
	...
	public function getcommonCompanyList_visit()
    {
	...
	public function filter_getCompanyList_forapi($id =0,$where=array(),$comp_id=0,$user_id=0,$process=0,$keyword=0,$action='data',$limit=-1,$offset=-1,$sort=-1)
    {
	...
	public function common_filter_getCompanyList($id =0,$where=array(),$comp_id=0,$user_id=0,$process=0,$keyword=0,$action='data',$limit=-1,$offset=-1,$sort=-1)
    {
