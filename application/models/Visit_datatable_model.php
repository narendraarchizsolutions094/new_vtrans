...
class Visit_datatable_model extends CI_Model{
    ...
    public function _get_datatables_query($postData){
        ...
        $all_reporting_ids    =   $this->common_model->get_categories($this->session->user_id);
        // print_r($_POST);
        $this->db->select($this->table.'.*,tbl_visit.created_at,enquiry.name,enquiry.status as enq_type,enquiry.Enquery_id,enquiry.company, tbl_visit.id as vids,tbl_company.company_name,enquiry.client_name');
